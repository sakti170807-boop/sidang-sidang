        ...
        <div class="flex items-center justify-between mb-4">
            ...
            @if($pendaftaran->status == 'submitted')
                <div class="flex gap-2">
                    <form action="{{ route('admin.pendaftaran.verify', $pendaftaran) }}" method="POST" 
            ...
            @if($pendaftaran->tanggal_submit)
                <div>
                    ...
                    <span class="font-medium text-gray-800 ml-2">
                        {{ \Carbon\Carbon::parse($pendaftaran->tanggal_submit)->format('d F Y, H:i') }}
                    </span>
                </div>
            @endif
            
            @if($pendaftaran->tanggal_verifikasi_pembimbing)
                <div>
                    <span class="text-gray-500">Verifikasi Pembimbing:</span>
                    <span class="font-medium text-gray-800 ml-2">
                        {{ \Carbon\Carbon::parse($pendaftaran->tanggal_verifikasi_pembimbing)->format('d F Y, H:i') }}
                    </span>
                </div>
            @endif
            
            @if($pendaftaran->tanggal_verifikasi_admin)
                <div>
                    <span class="text-gray-500">Verifikasi Admin:</span>
                    <span class="font-medium text-gray-800 ml-2">
                        {{ \Carbon\Carbon::parse($pendaftaran->tanggal_verifikasi_admin)->format('d F Y, H:i') }}
                    </span>
                </div>
            @endif
        </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <p class="font-medium text-gray-800">
                        {{ \Carbon\Carbon::parse($pendaftaran->jadwal->tanggal ?? $pendaftaran->jadwal->created_at)->format('d F Y') }}<br>
